...
| 4 | [`phase-4-file-io.md`](phase-4-file-io.md) | File I/O: open / save / save-as JSON & YAML, dirty state, close-tab, recent files | ✅ done (JSONL & YAML-multi added beyond original scope; round-trip tests partial) |
| 5 | [`phase-5-ux-polish.md`](phase-5-ux-polish.md) | UX polish: `displayText`, status bar, persisted column widths & expansion, search/filter — **split into 5.1–5.6 below** | ✅ done (stretch icons / match highlight deferred) |
...
| 5.2 | [`phase-5.2-display-formatting.md`](phase-5.2-display-formatting.md) | Raw `EditRole`, `ValueDelegate.displayText`, `ToolTipRole` for long values | ✅ done (icons deferred) |
| 5.3 | [`phase-5.3-status-bar-breadcrumb.md`](phase-5.3-status-bar-breadcrumb.md) | Permanent breadcrumb on selection + transient action messages | ✅ done |
| 5.4 | [`phase-5.4-persisted-view-state.md`](phase-5.4-persisted-view-state.md) | `QSettings`-backed per-file column widths / expansion / selection / font zoom | ✅ done |
| 5.5 | [`phase-5.5-search-filter.md`](phase-5.5-search-filter.md) | `TreeFilterProxy` + Ctrl+F search bar + delegate/tree-view audit for `mapToSource` | ✅ done |
...
| 6 | [`phase-6-tests.md`](phase-6-tests.md) | Test coverage: model unit tests, round-trip tests, GUI smoke tests | ⚠️ partial (401 tests pass; full delegate matrix + io round-trip property tests still pending) |
