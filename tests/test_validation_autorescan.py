...
def test_auto_rescan_on_value_edit_triggers_revalidation(qtbot):
    ...
    assert len(tab.issue_index) == 0, "should start valid"
    ...
    assert len(tab.issue_index) > 0, "should detect schema violation after auto-rescan"
...
def test_auto_rescan_off_mutation_does_not_trigger_revalidation(qtbot):
    ...
    assert len(tab.issue_index) == 0, "without auto-rescan, issue count should stay stale"
...
def test_rescan_now_updates_issues_when_auto_rescan_off(qtbot):
    ...
    assert len(tab.issue_index) == 0, "pre-condition: stale"
...
def test_auto_rescan_property_reflects_set(qtbot):
    ...
    assert tab.auto_rescan is False
    tab.set_auto_rescan(True)
    assert tab.auto_rescan is True
    tab.set_auto_rescan(False)
    assert tab.auto_rescan is False
