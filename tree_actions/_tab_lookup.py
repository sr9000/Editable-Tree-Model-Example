...
Lookup uses a marker base exported from ``documents.tab_protocols`` so
...
from documents.tab_protocols import JsonTabWidgetMarker, TreeActionsTabProtocol


def find_owning_tab(widget: object | None) -> TreeActionsTabProtocol | None:
    ...
    while node is not None:
        if isinstance(node, JsonTabWidgetMarker):
            return cast(TreeActionsTabProtocol, node)
