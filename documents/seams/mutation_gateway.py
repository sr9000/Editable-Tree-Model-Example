...
class DocumentMutationGateway:
    ...
    def commit_set_data(
        ...
        col = idx.column()
        if col == 0:
            return tab.editing.commands.push_rename(idx, value)
        if col == 1:
            return tab.editing.commands.push_change_type(idx, value)
        if col == 2:
            return tab.editing.commands.push_edit_value(idx, value)
        return False

    def push_edit_value(self, value_index: QModelIndex, new_value: Any, *, label: str = "edit value") -> bool:
        return self._tab.editing.commands.push_edit_value(value_index, new_value, label=label)

    def push_insert_rows(self, inserts: list, *, label: str = "insert", target_qname: str | None = None) -> bool:
        return self._tab.editing.commands.push_insert_rows(inserts, label=label, target_qname=target_qname)

    def push_remove_rows(self, indexes: list, *, label: str = "delete") -> bool:
        return self._tab.editing.commands.push_remove_rows(indexes, label=label)

    def push_move_rows_anchor(self, *args, **kwargs) -> bool:
        return self._tab.editing.commands.push_move_rows_anchor(*args, **kwargs)

    def push_move_rows(self, *args, **kwargs) -> bool:
        return self._tab.editing.commands.push_move_rows(*args, **kwargs)

    def push_sort_keys(self, *args, **kwargs) -> bool:
        return self._tab.editing.commands.push_sort_keys(*args, **kwargs)

    def push_switch_field_case(self, *args, **kwargs) -> bool:
        return self._tab.editing.commands.push_switch_field_case(*args, **kwargs)
    ...
    def push_sort_keys_at(self, parent_path: tuple[int, ...], *, recursive: bool = False) -> bool:
        ...
        if not parent_index.isValid():
            return False
        return self._tab.editing.commands.push_sort_keys(parent_index, recursive=recursive)
